...
// Login 登录
	...
	func (m *Btoken) Login(r *ghttp.Request) {
...
// Logout 登出
	...
	func (m *Btoken) Logout(r *ghttp.Request) {
...
// AuthMiddleware 认证拦截
	...
	func (m *Btoken) authMiddleware(r *ghttp.Request) {
		urlPath := r.URL.Path
...
// GetTokenData 通过token获取对象
	...
	func (m *Btoken) GetTokenData(r *ghttp.Request) Resp {
		respData := m.getRequestToken(r)
		if respData.Success() {
			// 验证token
...
// return true 需要认证
	...
	func (m *Btoken) AuthPath(ctx context.Context, urlPath string) bool {
		// 去除后斜杠
...
*/
func (m *Btoken) GenTokenWithMultiLogin(ctx context.Context, userKey string, data interface{}) Resp {
...
func (m *Btoken) GetRequestToken(r *ghttp.Request) Resp {
...
// genToken 生成Token
func (m *Btoken) GenToken(ctx context.Context, userKey string, data interface{}) Resp {
...
// validToken 验证Token
func (m *Btoken) ValidToken(ctx context.Context, token string) Resp {
	if token == "" {
		return Unauthorized(MsgErrTokenEmpty, "")
	}
...
func (m *Btoken) GetToken(ctx context.Context, userKey string) Resp {
	cacheKey := m.CacheKey + userKey
...
// RemoveToken 删除Token
func (m *Btoken) RemoveToken(ctx context.Context, token string) Resp {
	decryptToken := m.DecryptToken(ctx, token)
	if !decryptToken.Success() {
		return decryptToken
	}
...
func (m *Btoken) EncryptToken(ctx context.Context, userKey string, uuid string) Resp {
	if userKey == "" {
...
func (m *Btoken) DecryptToken(ctx context.Context, token string) Resp {
	if token == "" {
